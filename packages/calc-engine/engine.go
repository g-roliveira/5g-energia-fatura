...
// Calculate é a função canônica do motor. Dada uma entrada, produz o
// resultado de forma determinística e sem efeitos colaterais.
//
// Algoritmo:
//
//  1. Coleta os itens por Type. TUSD fio + TUSD energia + (opcional)
//     Energia Injetada são tratados como preços unitários (R$/kWh).
//     Bandeira e IP Coelba são valores fixos já em R$.
//
...
//
//  3. Subtotal "sem desconto" de energia = consumo_liquido * (tusd_fio + tusd_te).
//     Subtotal "com desconto" = subtotal_sem_desconto * Contract.DescontoPct.
//
//  4. Bandeira: sempre somada em "sem desconto". Em "com desconto" só
//     sofre desconto se Contract.BandeiraComDesconto=true.
//
//  5. IP Coelba: sempre somada em "sem desconto". Nunca tem desconto.
//
//  6. IP da Usina: não aparece na fatura Coelba. Adicionada apenas no
//     lado "com desconto" conforme Contract.IPFaturamentoMode.
//
...
func computeIPUsina(c Contract, subtotalEnergiaCom decimal.Decimal) decimal.Decimal {
